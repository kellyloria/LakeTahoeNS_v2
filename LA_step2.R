...
library(patchwork)
library(plotly)
#source("D:/IADO/UNR/NoahPPR/scripts/helper_functions.R")
# get.abrev(), fillHoles(), create.plots()
...
sonde = list.files(paste("/Users/kellyloria/Desktop/Kelly/Alder/Finalinputs3M/",sep=""), full.names = T) %>%
  lapply(read_csv) %>%
  bind_rows()
...
if(lake == "castle"){
  data <- sonde %>% 
    mutate(datetime = ymd_hms(datetime,tz=tz)) %>%
    filter((year == 2015 & yday >= 213 & yday <=269) |
             (year==2016 & yday >=214 & yday <=274) |
             (year == 2018 & yday >= 213 & yday <=273) |
             (year == 2019 & yday >=213 & yday <= 273)) %>% 
    drop_na()  
}
...
if(length(years) == 1) {
  sonde_check %>%
    write_csv(paste("/Users/kellyloria/Desktop/Kelly/Alder/Modelinputs3M/sonde_prep_",lake,"_",years,".csv",sep =""))
} else {
  sonde_check %>%
    write_csv(paste("/Users/kellyloria/Desktop/Kelly/Alder/Modelinputs3M/sonde_prep_",lake,"_",min(years),"_",max(years),".csv",sep =""))
}
...
if(length(years)>1) {
  stan_rdump(c("o2_freq","o2_obs","o2_eq","light","temp","wspeed","map_days","obs_per_series","days_per_year",
             ...
             file=paste("/Users/kellyloria/Desktop/Kelly/Alder/Modelinputs3M/",lake,"_",min(years),"_",max(years),"_sonde_list.R",sep=""))
} else {
  stan_rdump(c("o2_freq","o2_obs","o2_eq","light","temp","wspeed","map_days","obs_per_series","days_per_year",
               "obs_per_day", "z","k","n_obs","n_series","n_days","n_years"),
             file=paste("/Users/kellyloria/Desktop/Kelly/Alder/Modelinputs3M/",lake,"_",years,"_sonde_list.R",sep=""))
}
